...
dbContext.prototype.openConnection = function(){
    let self = this;
    return this.mssql.connect(config).then(function(connection){
...
dbContext.prototype.queryDatabaseList = Q.async(function* (sql, obj){
    try
    {
        ...
        // query database
        let pool = yield new this.mssql.ConnectionPool(config).connect();
...
dbContext.prototype.queryDatabaseItem = Q.async(function* (sql, obj){
    try
    {
        if(this.logSql) console.log(sql);
